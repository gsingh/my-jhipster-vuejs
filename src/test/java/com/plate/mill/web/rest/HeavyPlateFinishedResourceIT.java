...
public class HeavyPlateFinishedResourceIT {
    ...
    @Autowired
    private HeavyPlateFinishedService heavyPlateFinishedService;

    /**
     * This repository is mocked in the com.plate.mill.repository.search test package.
     *
     * @see com.plate.mill.repository.search.HeavyPlateFinishedSearchRepositoryMockConfiguration
     */
    @Autowired
    private HeavyPlateFinishedSearchRepository mockHeavyPlateFinishedSearchRepository;
    ...
    public void setup() {
        ...
        final HeavyPlateFinishedResource heavyPlateFinishedResource = new HeavyPlateFinishedResource(heavyPlateFinishedService);
    ...
    public void createHeavyPlateFinished() throws Exception {
        ...
        // Validate the HeavyPlateFinished in Elasticsearch
        verify(mockHeavyPlateFinishedSearchRepository, times(1)).save(testHeavyPlateFinished);
    }
    ...
    public void createHeavyPlateFinishedWithExistingId() throws Exception {
        ...
        // Validate the HeavyPlateFinished in Elasticsearch
        verify(mockHeavyPlateFinishedSearchRepository, times(0)).save(heavyPlateFinished);
    }
    ...
    public void updateHeavyPlateFinished() throws Exception {
        // Initialize the database
        heavyPlateFinishedService.save(heavyPlateFinished);
        // As the test used the service layer, reset the Elasticsearch mock repository
        reset(mockHeavyPlateFinishedSearchRepository);
    ...
    public void deleteHeavyPlateFinished() throws Exception {
        ...
        // Validate the HeavyPlateFinished in Elasticsearch
        verify(mockHeavyPlateFinishedSearchRepository, times(1)).deleteById(heavyPlateFinished.getId());
    }

    @Test
    @Transactional
    public void searchHeavyPlateFinished() throws Exception {
        // Initialize the database
        heavyPlateFinishedService.save(heavyPlateFinished);
        when(mockHeavyPlateFinishedSearchRepository.search(queryStringQuery("id:" + heavyPlateFinished.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(heavyPlateFinished), PageRequest.of(0, 1), 1));
        // Search the heavyPlateFinished
        restHeavyPlateFinishedMockMvc.perform(get("/api/_search/heavy-plate-finisheds?query=id:" + heavyPlateFinished.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(heavyPlateFinished.getId().intValue())))
            .andExpect(jsonPath("$.[*].hPFinishedDate").value(hasItem(DEFAULT_H_P_FINISHED_DATE.toString())))
