...
public class ShiftManagerResourceIT {
     ...
     *
     * @see com.plate.mill.repository.search.ShiftManagerSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void setup() {
        ...
        final ShiftManagerResource shiftManagerResource = new ShiftManagerResource(shiftManagerService);
    ...
    public void createShiftManager() throws Exception {
        ...
        // Validate the ShiftManager in Elasticsearch
        verify(mockShiftManagerSearchRepository, times(1)).save(testShiftManager);
    }
    ...
    public void createShiftManagerWithExistingId() throws Exception {
        ...
        // Validate the ShiftManager in Elasticsearch
        verify(mockShiftManagerSearchRepository, times(0)).save(shiftManager);
    }
    ...
    public void updateShiftManager() throws Exception {
        // Initialize the database
        shiftManagerService.save(shiftManager);
        // As the test used the service layer, reset the Elasticsearch mock repository
        reset(mockShiftManagerSearchRepository);
    ...
    public void deleteShiftManager() throws Exception {
        ...
        // Validate the ShiftManager in Elasticsearch
        verify(mockShiftManagerSearchRepository, times(1)).deleteById(shiftManager.getId());
    }

    @Test
    @Transactional
    public void searchShiftManager() throws Exception {
        // Initialize the database
        shiftManagerService.save(shiftManager);
        when(mockShiftManagerSearchRepository.search(queryStringQuery("id:" + shiftManager.getId())))
            .thenReturn(Collections.singletonList(shiftManager));
        // Search the shiftManager
        restShiftManagerMockMvc.perform(get("/api/_search/shift-managers?query=id:" + shiftManager.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(shiftManager.getId().intValue())))
