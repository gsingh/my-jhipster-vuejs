...
public class EventOfPlateMillResourceIT {
    ...
    @Autowired
    private EventOfPlateMillService eventOfPlateMillService;

    /**
     * This repository is mocked in the com.plate.mill.repository.search test package.
     *
     * @see com.plate.mill.repository.search.EventOfPlateMillSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void setup() {
        ...
        final EventOfPlateMillResource eventOfPlateMillResource = new EventOfPlateMillResource(eventOfPlateMillService);
    ...
    public void createEventOfPlateMill() throws Exception {
        ...
        // Validate the EventOfPlateMill in Elasticsearch
        verify(mockEventOfPlateMillSearchRepository, times(1)).save(testEventOfPlateMill);
    }
    ...
    public void createEventOfPlateMillWithExistingId() throws Exception {
        ...
        // Validate the EventOfPlateMill in Elasticsearch
        verify(mockEventOfPlateMillSearchRepository, times(0)).save(eventOfPlateMill);
    }
    ...
    public void updateEventOfPlateMill() throws Exception {
        // Initialize the database
        eventOfPlateMillService.save(eventOfPlateMill);
        // As the test used the service layer, reset the Elasticsearch mock repository
        reset(mockEventOfPlateMillSearchRepository);
    ...
    public void deleteEventOfPlateMill() throws Exception {
        ...
        // Validate the EventOfPlateMill in Elasticsearch
        verify(mockEventOfPlateMillSearchRepository, times(1)).deleteById(eventOfPlateMill.getId());
    }

    @Test
    @Transactional
    public void searchEventOfPlateMill() throws Exception {
        // Initialize the database
        eventOfPlateMillService.save(eventOfPlateMill);
        when(mockEventOfPlateMillSearchRepository.search(queryStringQuery("id:" + eventOfPlateMill.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(eventOfPlateMill), PageRequest.of(0, 1), 1));
        // Search the eventOfPlateMill
        restEventOfPlateMillMockMvc.perform(get("/api/_search/event-of-plate-mills?query=id:" + eventOfPlateMill.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(eventOfPlateMill.getId().intValue())))
            .andExpect(jsonPath("$.[*].eventDate").value(hasItem(DEFAULT_EVENT_DATE.toString())))
